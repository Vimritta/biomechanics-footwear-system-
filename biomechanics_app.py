...
def set_white_theme():
    ...
    select, textarea, input { background-color: white !important; color: black !important; }
    .stButton>button { background-color: #d9c2f0 !important; color: black !important; border-radius:6px; font-weight:600;}
...
# ---------------------------
    ...
    activity_label = st.selectbox("Select your Daily Activity Level", ["Low (mostly sitting)","Moderate (walking/standing sometimes)","High (frequent walking/running)"], index=1)
...
# ---------------------------
    ...
    st.subheader("Select Voice Language for Recommendation")
    tts_lang = st.radio("Language", ["English","Tamil","Sinhala"], index=["English","Tamil","Sinhala"].index("English"))
    lang_code = {"English":"en","Tamil":"ta","Sinhala":"si"}[tts_lang]
    st.session_state.tts_lang = lang_code
    ...
    if st.session_state.analyze_clicked:
        st.markdown(f"<img src='{os.path.join(IMAGE_DIR,'walking.gif')}' width='220' style='border-radius:8px;'/>", unsafe_allow_html=True)
        # Speak recommendation + justification + tip
        tips = [
        ...
        greetings = {"en":"Hello!","ta":"வணக்கம்!","si":"ආයුබෝවන්!"}[st.session_state.tts_lang]
        speech_text = f"{greetings} Recommendation: {brand}. Material: {material}. Justification: {justification}. Tip of the Day: {tip_text}"
        speak_multilang(speech_text, lang=st.session_state.tts_lang)
    ...
    with rec_col1:
        ...
        st.markdown(f"<div style='background-color:#d2b48c;border-left:6px solid #8b6f47;padding:10px 14px;border-radius:8px;margin-top:8px;font-weight:600;color:#222;'>💬 Justification: {html_mod.escape(justification)}</div>", unsafe_allow_html=True)
        st.markdown(f"<div style='background-color:#fff9c4;border-left:6px solid #ffd54f;padding:10px 14px;border-radius:8px;margin-top:8px;font-weight:600;color:#333;'>💡 Tip of the Day: {tip_text}</div>", unsafe_allow_html=True)
        # Download
        ...
        st.markdown(f"<a download='footfit_recommendation.txt' href='data:text/plain;base64,{b64}' style='background-color:#ff4da6;color:white;padding:10px 14px;border-radius:8px;text-decoration:none;font-weight:bold;display:inline-block;'>📄 Download Recommendation (txt)</a>", unsafe_allow_html=True)

    with rec_col2:
        st.subheader("👟 Virtual Shoe Wall")
        sample_map={"Running shoes":["running1.png","running2.png"],"Cross-training shoes":["cross1.png","cross2.png"],"Casual/fashion sneakers":["casual1.png","casual2.png"],"Sandals or slippers":["sandal1.png","sandal2.png"]}
